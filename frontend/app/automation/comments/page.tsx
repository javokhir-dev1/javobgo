...
export default function AutomationCommentsPage() {
  ...
  const save = async () => {
    ...
    if (form.replyEnabled && form.replyAgentId === -1) { setSaveError('Izohga javob: avval AI agent yarating'); return; }
    if (form.dmEnabled && form.dmAgentId === -1) { setSaveError('DM yuborish: avval AI agent yarating'); return; }
    const validReply = (form.replyTemplates || []).filter(t => t?.trim());
    const validDm = (form.dmTemplates || []).filter(t => t?.trim());
    if (form.replyEnabled && !form.replyAgentId && validReply.length < 3) { setSaveError('Izohga javob: AI agentsiz kamida 3 ta shablon kiritilishi shart'); return; }
    if (form.dmEnabled && !form.dmAgentId && validDm.length < 3) { setSaveError('DM yuborish: AI agentsiz kamida 3 ta shablon kiritilishi shart'); return; }
    setSaving(true);
    try {
  ...
  return (
    <div className="h-full overflow-y-auto bg-background text-on-surface p-4 md:p-6">
